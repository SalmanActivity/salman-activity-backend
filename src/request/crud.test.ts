...
describe('request crud endpoint test', () => {
  ...
  before((done) => {
    const promiseAdmin = server.post('/api/v1/auth/login')
    .send({
    ...
    const promiseUser = server.post('/api/v1/auth/login')
    .send({
  ...
  const checkSpecificRequest = (document) => {
    return res => {
