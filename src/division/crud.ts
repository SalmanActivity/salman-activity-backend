...
const filterFieldOne = crudUtil.filterOne.fields(['id', 'name', 'enabled']);
...
const validate = async (updating, userInput) => {
...
export function updateOneDivision (divisionAccessor:DivisionAccessor = new DivisionMongoAccessor()) {
  return crudUtil.updateOne({
    ...
    updateOne: (validatedData, context) => {
      const updateData = Object.assign(context.updating, validatedData);
