...
async function filterUserByRole(user, context) {
  ...
  if (curUser.id != user.id && !curUser.admin)
    try {
      let currDivId = curUser.division.id
      let userDivId = user.division.id
...
let filterField = crudUtil.filterOne.fields(['id', 'name', 'username', 'email', 'division', 'enabled', 'admin'])
...
let validateUserInput = async (updatingUser, userInput, userAccessor:UserAccessor,
